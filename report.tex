...
\newcommand{\secondmarker}{Dr. Benjamin Hou}
...
\begin{table}[]
    ...
    \bottomrule
    \end{tabular}
    \captionsetup{type=table}\captionof{table}{Learning rate hyperparameter tuning results for standard MOCO training}
...
\begin{table}[]
    ...
    \bottomrule
    \end{tabular}
    \captionsetup{type=table}\captionof{table}{Comparison between standard MOCO classification performance and }
...
\begin{table}[]
    ...
    \bottomrule
    \end{tabular}
    \captionsetup{type=table}\captionof{table}{Classification performance of standard MOCO trained encoder with different classification head complexity}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figures/naiveMOCO_cm.png}
...
Recall & $0.8862$ ($+0.0678$) & $0.6238$ ($+0.3060$) & $0.8992$ ($-0.0260$) \\
    \bottomrule
    \end{tabular}
    \captionsetup{type=table}\captionof{table}{Per-class precision and recall values of the model trained using standard MOCO learning. The values in the brackets show the difference in metric scores from the model trained end-to-end on the dev-training dataset. }
...
\begin{table}[]
    ...
    \bottomrule
    \end{tabular}
    \captionsetup{type=table}\captionof{table}{LR annealing stage number hyperparameter tuning results for standard MOCO training. A single LR annealing stage consists of 20 epochs of training}
...
% The largest improvement in precision score is achieved by the smooth muscle image class ($+0.0422$), but this value is smaller than the average improvement in precision score achieved by the standard MOCO learning. The largest 

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figures/supervisedMOCO_cm.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figures/gradcam1_curve.png}
